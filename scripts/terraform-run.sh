...
# The Proxmox API token is fetched from Bitwarden SM and exported as
# PROXMOX_VE_API_TOKEN before the terraform command runs.
#
# Usage:
#   scripts/terraform-run.sh <terraform args...>
#
# Examples:
#   scripts/terraform-run.sh -chdir=terraform/proxmox init
#   scripts/terraform-run.sh -chdir=terraform/proxmox plan
#   scripts/terraform-run.sh -chdir=terraform/proxmox apply -target=proxmox_virtual_environment_container.technitium
#
# Required environment variables:
#   BWS_ACCESS_TOKEN                     Bitwarden SM machine account token
#   PROXMOX_API_TOKEN_BWS_SECRET_ID      BWS secret UUID for the Proxmox API token
#                                        (format: "root@pam!terraform=<uuid>")
#
# Optional environment variables:
#   PROXMOX_VE_ENDPOINT    default: https://10.0.10.11:8006/
#   PROXMOX_VE_INSECURE    default: true
#   PROXMOX_VE_API_TOKEN   if set, BWS lookup is skipped
