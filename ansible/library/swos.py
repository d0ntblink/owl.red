...
try:
    ...
    import swos.platform as _swos_platform_module
    HAS_SWOS_API = True
except ImportError:
    HAS_SWOS_API = False
...
def run_module():
    module_args = dict(
        ...
        force_platform=dict(type='str', required=False, default=None, choices=['swos', 'swos-lite']),
    )
    ...
    # CSS326 runs full SwOS but is misdetected as swos-lite by the library.
    # Force SWOS for CSS326 to enable VLAN names and other SwOS-only fields.
    force_platform = module.params.get('force_platform')
    try:
        if force_platform == 'swos':
            _swos_platform_module.detect_platform = lambda url, u, p: PlatformType.SWOS
            platform_type = PlatformType.SWOS
        elif force_platform == 'swos-lite':
            _swos_platform_module.detect_platform = lambda url, u, p: PlatformType.SWOS_LITE
            platform_type = PlatformType.SWOS_LITE
        else:
